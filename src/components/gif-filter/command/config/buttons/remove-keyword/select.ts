...
export default {
    data: {
        ...
        custom_id: 'remove_keyword_select',
        options: [],
        min_values: 1,
        max_values: 1,
    },
    async execute({ data: interaction, api }) {
        const keyword = interaction.data.values[0]!;
        ...
        if (guildConfig.keywords.includes(keyword)) {
            guildConfig.keywords.splice(
                guildConfig.keywords.indexOf(keyword),
                1,
            );
            await config.write();
        }
        ...
        await api.interactions.editReply(
            ...
            (await getCreateConfigMessage())(guildConfig, {
                content: `Successfully removed the blacklisted keyword \`${keyword}\`.`,
            }),
        );
    },
